...
const createFlavorModal = () => {
    imageData = ''
    flavorModal.querySelector('h1').textContent = `Створити ${FLAVOR}`
...
const createFlavorRow = flavor => {
    ...
    tr.onpointerup = e => {
        ...
        }).catch(() => showMessage('error', getErrorMessage(FLAVOR)))
    }

    editAction.onpointerup = () => {
        imageData = flavor.imageData
        flavorModal.querySelector('h1').textContent = `Редагувати ${FLAVOR}`
...
const createFlavor = () => {
    ...
    post('Flavor', flavor).then(response => {
        showMessage('success', createSuccessMessage(FLAVOR))
        ...
        flavorModal.style.display = ''
        showMessage('error', createErrorMessage(FLAVOR))
    })
}
...
const editFlavor = (id, oldRow, date) => {
    ...
    put('Flavor', flavor).then(response => {
        showMessage('info', updateSuccessMessage(FLAVOR))
        ...
        flavorModal.style.display = ''
        showMessage('error', updateErrorMessage(FLAVOR))
    })
}
...
const viewFlavorTemplates = () => get(`FlavorTemplate/${loginInfo.companyId}`).then(response => {
