let inventoriesTable, inventoryFilters, inventories, inventoryStocks
...
const showInventoryInfo = e => {
    ...
    get(`Inventory/${loginInfo.companyId}`).then(response => {
        ...
        replaceLoadIcons()
    }).catch(() => showMessage('error', getErrorMessage('інвентаризації')))
...
const createInventory = () => {
    ...
    post('Inventory', inventory).then(response => {
        ...
        inventoryModal.style.display = ''
        showMessage('error', createErrorMessage(INVENTORY))
    })
}
