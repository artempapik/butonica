...
const showProductInfo = e => {
    ...
    get(`Product/${loginInfo.companyId}`).then(response => {
        ...
        products.forEach(fillProductsTable)
        replaceLoadIcons()
    ...
    if (productPricesHistoryLineChart) {
        productPricesHistoryLineChart.destroy()
    }
...
const createProductModal = () => {
    ...
    $(productModal.querySelector('select')).select2(select2NoSearch('Обрати категорію'))
    ...
    productModal.querySelector('div').scroll(0, 0)
}
...
const createProductRow = product => {
    ...
    if (loginInfo.title < 2) {
        editAction.onpointerup = () => {
            ...
            $(productModal.querySelector('select')).select2(select2NoSearch())
...
const filterProducts = () => {
    ...
    productsTable.innerHTML = productsTable.querySelector('tbody').innerHTML
    filteredProducts.forEach(fillProductsTable)
}
