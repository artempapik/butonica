let productsTable, productsInput, productsFiltersBlock, products
const PRODUCT = 'товар'
...
const createProductModal = () => {
    imageData = ''
    productModal.querySelector('h1').textContent = `Створити ${PRODUCT}`
...
const createProductRow = product => {
    ...
    if (loginInfo.title < 2) {
        editAction.onpointerup = () => {
            ...
            productModal.querySelector('h1').textContent = `Редагувати ${PRODUCT}`
        ...
        deleteAction.onpointerup = () => {
            if (confirm(`Ви дійсно бажаєте видалити ${PRODUCT} "${product.name}"?`)) {
                delete product.category
    
                remove('Product', product).then(() => {
                    showMessage('info', deleteSuccessMessage(PRODUCT))
                ...
                }).catch(() => showMessage('error', deleteErrorMessage(PRODUCT)))
            }
        }
    }
...
const createProduct = () => {
    ...
    post('Product', product).then(response => {
        showMessage('success', createSuccessMessage(PRODUCT))
        product.id = response
        ...
        productsInput.style.display = 'block'
        productsFiltersBlock.style.display = 'flex'
    }).catch(() => showMessage('error', createErrorMessage(PRODUCT)))
}
