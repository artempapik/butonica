...
const showSaleInfo = e => {
    get(`Shift/last/${loginInfo.employeeId}`).then(response => {
        ...
        checkoutClients.querySelector('span:last-child').onpointerup = () => checkoutClients.querySelector('select').value = ''
...
const createShift = (shift = null, product = null) => {
    ...
    close.onpointerup = () => {
        if (!shifts[activeShiftIndex].products.length || (shifts[activeShiftIndex].products.length && confirm('Ви дійсно бажаєте видалити продаж?'))) {
            closeShift(false)
        }
    }
...
const performCashRegisterOperation = () => {
    ...
    const sumInput = cashRegisterOperationsModal.querySelector('input[type=number]')
    ...
    post('Shift/operation', cashRegisterOperation).then(() => {
        cashRegisterOperationsModal.style.display = ''
...
const endEmployeeShift = () => {
    if (confirm('Ви дійсно бажаєте закрити зміну?')) {
        put('Shift', { id: shiftId }).then(() => {
