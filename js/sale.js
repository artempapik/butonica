...
const showSaleInfo = e => {
    get(`Shift/last/${loginInfo.employeeId}`).then(response => {
        ...
        clientModal.querySelector('button').onpointerup = () => {
            ...
            }).catch(() => showMessage('error', createErrorMessage(CLIENT)))
        }
...
const createSaleOrder = saleOrderType => {
    ...
    hideModal(saleOrderModal)

    post('Order', order).then(() => {
        closeShift()
        showMessage('success', createSuccessMessage(ORDER))
    }).catch(() => showMessage('error', createErrorMessage(ORDER)))
}
...
const performCashRegisterOperation = () => {
    ...
    hideModal(cashRegisterOperationsModal)

    post('Shift/operation', cashRegisterOperation)
        .then(() => showMessage('success', 'Операція проведена'))
        .catch(e => {
            if (e.message === '403') {
