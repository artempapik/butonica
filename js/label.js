...
const createLabelRow = label => {
    ...
    deleteAction.onpointerup = () => {
        if (confirm(`Ви дійсно бажаєте видалити мітку "${label.name}"?`)) {
...
const createLabel = () => {
    ...
    post('Label', label).then(response => {
        ...
        labelsTable.style.display = 'block'
    }).catch(() => showMessage('error', createErrorMessage('мітку')))
}
...
const editLabel = (id, oldRow) => {
    ...
    }).catch(() => showMessage('error', updateErrorMessage('мітку')))
}
