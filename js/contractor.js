...
const createContractorRow = contractor => {
    ...
    deleteAction.onpointerup = () => {
        if (confirm(`Ви дійсно бажаєте видалити постачальника "${contractor.name}"?`)) {
...
const createContractor = () => {
    ...
    post('Contractor', contractor).then(response => {
        ...
        contractorsTable.style.display = 'block'
    }).catch(() => showMessage('error', createErrorMessage('постачальника')))
}
...
const editContractor = (oldContractor, oldRow) => {
    ...
    }).catch(() => showMessage('error', updateErrorMessage('постачальника')))
}
