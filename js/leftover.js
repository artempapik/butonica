...
const showLeftoverInfo = e => {
    ...
    get(`Stock/names/${loginInfo.companyId}`).then(response => fillLeftoverFilters(leftoverStockFilters, response, 'name', 'stocks'))
}
...
const fillLeftoversTable = leftoverProducts => {
    ...
    if (!leftoverProducts.length) {
        ...
        leftoversTable.append(emptyDataDiv)
        return
    }
...
const sortLeftoverByStock = () => changeSortIconAndUpdateFilters(2, 'sortByStock')
