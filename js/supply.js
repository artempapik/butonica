...
const getSupplies = month => {
    ...
    get(`Supply/${loginInfo.companyId}/${month || new Date().getMonth() + 1}`).then(response => {
        ...
        response.forEach(fillSuppliesTable)
...
const showSupplyInfo = e => {
    ...
    get(`Contractor/ids-names/${loginInfo.companyId}`).then(response => {
        const supplyContractorsList = supplyModal.querySelector('.supply-contractor')
        supplyContractorsList.innerHTML = ''

        for (const contractor of response) {
            const option = document.createElement('option')
            option.text = contractor.name
            option.dataset.id = contractor.id
            supplyContractorsList.append(option)
        }
        
        $(supplyContractorsList).val('').select2(select2NoResults('Обрати постачальника'))
    })
    ...
    get(`Stock/ids-names/${loginInfo.companyId}`).then(response => {
        ...
        $(supplyStocks).select2(select2NoSearch('Обрати склад'))
    })

    get(`Product/ids-names/${loginInfo.companyId}`).then(response => supplyProducts = response)
}
...
const createSupplyRow = supply => {
    ...
    editAction.onpointerup = () => {
        ...
        for (const product of supply.products.slice(1)) {
            ...
            supplyProductAmount.inputMode = 'numeric'
...
const createSupply = () => {
    ...
    const contractorElement = supplyModal.querySelector('.supply-contractor')
    ...
    const allProductChoices = [...supplyModal.querySelectorAll('.supply-products select')].map(i => i.value)
    ...
    const supply = {
        date,
        contractorId: +contractorElement.selectedOptions[0].dataset.id,
...
const addSupplyProduct = () => {
    ...
    for (const product of supplyProducts) {
        ...
        supplyProductSelect.append(option)
    }
    
    supplyProductSelect.value = ''
