let suppliesTable, supplies, supplyProducts
const SUPPLY = 'поставку'
...
const showSupplyInfo = e => {
    ...
    get(`Supply/${loginInfo.companyId}`).then(response => {
        supplies = response
        ...
        supplies.forEach(s => fillSuppliesTable(s))
...
const createSupplyModal = () => {
    supplyModal.querySelector('h1').textContent = `Створити ${SUPPLY}`
...
const createSupplyRow = supply => {
    ...
    tr.onpointerup = e => {
        ...
        hideBodyOverflow()
        supplyInfoModal.querySelector('.supply-date').textContent = formatDate(supply.date)
        ...
        get(`Supply/supply/${supply.id}`).then(response => {
            supplyInfoModal.querySelector('.supply-paid-sum').textContent = response.paidSum ? response.paidSum.toFixed(2) + ' грн' : ''
            supplyInfoModal.querySelector('.supply-pay-date').textContent = formatDate(response.payDate)
            supplyInfoModal.querySelector('.supply-comment').textContent = response.comment
        ...
        }).catch(() => showMessage('error', getErrorMessage(SUPPLY)))
    }

    editAction.onpointerup = () => {
        supplyModal.querySelector('h1').textContent = `Редагувати ${SUPPLY}`
    ...
    tr.append(
        createTd(formatDate(supply.date)),
...
const createSupply = () => {
    ...
    if (!dateElement.value) {
        showMessage('error', 'Введіть дату поставки')
        return
    }
...
const sortSupplies = () => {
    ...
    const sortIcon = suppliesTable.querySelector('tbody tr td:first-child').querySelectorAll('span').item(1)
    const sortedSupplies = [...supplies]

    if (sortIcon.innerHTML === 'unfold_more') {
        sortIcon.innerHTML = 'arrow_drop_up'
        sortedSupplies.sort((a, b) => new Date(a.date) - new Date(b.date))
    } else if (sortIcon.innerHTML === 'arrow_drop_up') {
        sortIcon.innerHTML = 'arrow_drop_down'
        sortedSupplies.sort((b, a) => new Date(a.date) - new Date(b.date))
    } else {
        sortIcon.innerHTML = 'unfold_more'
    }

    sortedSupplies.forEach(s => fillSuppliesTable(s))
}
