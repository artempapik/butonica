...
const firstAllOrderPage = e => {
    ...
    get(`Order/${loginInfo.companyId}/${currentPage}`).then(response => {
...
const showAllOrderInfo = e => {
    ...
    get(`Order/${loginInfo.companyId}/1`).then(response => {
        if (response.length) {
...
const createInternetOrder = saleOrderType => {
    ...
    hideModal(internetOrderModal)

    post('Order/internet', order)
        .then(() => showMessage('success', createSuccessMessage('online-замовлення')))
        .catch(() => showMessage('error', createErrorMessage('online-замовлення')))
}
