...
const createEmployeeRow = employee => {
    ...
    tr.onpointerup = e => {
        ...
        employeeInfoModal.querySelector('.employee-email').textContent = employee.email
    ...
    if (loginInfo.title === 0) {
        editAction.onpointerup = () => {
            ...
            employeeModal.querySelector('.employee-email').value = employee.email
...
const createEmployee = () => {
    const emailElement = employeeModal.querySelector('.employee-email')
    const email = emailElement.value.trim()

    if (!email) {
        showMessage('error', 'Введіть email працівника')
        return
    }
    ...
    post('Employee', employee).then(response => {
        ...
        if (e.message === '406') {
            showMessage('error', 'Такий email вже існує в системі')
            return
        }
