...
const createCategoryModal = () => {
    categoryModal.querySelector('h1').textContent = `Створити ${CATEGORY}`
...
const createCategoryRow = category => {
    ...
    if (loginInfo.title < 2) {
        editAction.onpointerup = () => {
            categoryModal.querySelector('h1').textContent = `Редагувати ${CATEGORY}`
        ...
        deleteAction.onpointerup = () => {
            if (confirm(`${CONFIRM_DELETE_TEXT} ${CATEGORY} "${category.name}"?\nУСІ пов'язані з нею товари також будуть видалені.`)) {
                remove('Category', category).then(() => {
                    showMessage('info', deleteSuccessMessage(CATEGORY))
                ...
                }).catch(() => showMessage('error', deleteErrorMessage(CATEGORY)))
            }
        }
    }
...
const createCategory = () => {
    ...
    post('Category', category).then(response => {
        showMessage('success', createSuccessMessage(CATEGORY))
        category.id = response
        categories.push(category)
        fillCategoriesTable(category)
        categoriesInput.style.display = 'block'
        categoriesTable.style.display = 'block'
    }).catch(() => showMessage('error', createErrorMessage(CATEGORY)))
}
...
const editCategory = (id, oldRow) => {
    ...
    put('Category', category).then(() => {
        showMessage('info', updateSuccessMessage(CATEGORY))
        const newRow = createCategoryRow(category)
        categoriesTable.replaceChild(newRow, oldRow)
    }).catch(() => showMessage('error', updateErrorMessage(CATEGORY)))
}
